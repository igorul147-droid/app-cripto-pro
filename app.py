...
from crypto_app.utils import TZ_LOCAL, add_indicators, compute_dtick_for_range, default_visible_candles, fmt_price, to_local_naive
...
with st.sidebar:
    ...
    # PRO: refresh rápido só para redesenhar (WS alimenta os dados)
    ...
    candles_visible = st.slider("Zoom inicial (candles)", 50, min(1200, candles_to_load), min(default_visible_candles("1h"), candles_to_load), 10)
...
# Limpa stores não selecionados
for sym in list(rt_stores.keys()):
    if sym not in [symbol_compact(m) for m in moedas]:
        try:
            rt_stores[sym].stop()
        except Exception:
            pass
...
# Cria/Start stores dos selecionados
for moeda in moedas:
    ...
    if sym not in rt_stores:
        ...
        # se usuário aumentar histórico, recarrega base (uma vez)
        # (opcional — pode deixar assim para não “pular” zoom)
        pass
...
for moeda in moedas:
    ...
    with col_main:
        ...
        meme_coins = set()  # opcional: mantenha sua lógica se quiser
        k1.metric(f"💰 Preço {moeda}", fmt_price(moeda, ultimo, meme_coins), f"{var_pct:.2f}%")
